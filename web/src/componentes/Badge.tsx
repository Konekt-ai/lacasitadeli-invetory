...
const ORDEN: Condicion[] = [
  'descontinuado', 'sin_stock', 'desfasado', 'bajo_stock', 'sin_alta', 'duplicado_probable',
...
export function Badge({ condicion, grande = false }: { condicion: Condicion; grande?: boolean }) {
  return (
    <span className={`chip ${grande ? 'px-3 py-1.5 text-sm' : ''} ${COLORES_CONDICION[condicion] ?? 'bg-surface-variant'}`}>
      {ETIQUETAS_CONDICION[condicion] ?? condicion}
    </span>
  );
}

export function Badges({ condiciones, max = 4, grande = false }: { condiciones: Condicion[]; max?: number; grande?: boolean }) {
  ...
  return (
    <span className="flex flex-wrap items-center gap-1.5">
      {visibles.map(c => <Badge key={c} condicion={c} grande={grande} />)}
